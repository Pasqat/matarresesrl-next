...
export function eventSchema({
  ...
  return {
    ...
    location,
    organizer: organizer
      ? {'@type': 'Organization', name: organizer.title || COMPANY.name, url: organizer.website || DOMAIN}
      : {'@id': ORG_ID},
  }
}

/** Progetto/realizzazione. */
export function creativeWorkSchema({title, description, slug, image, categories = []}) {
  return {
    '@context': 'https://schema.org',
    '@type': 'CreativeWork',
    name: title,
