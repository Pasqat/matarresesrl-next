...
fatti sul sito, (2) azioni off-site da svolgere manualmente, (3) note GEO, (4)
...
- **Pagina `/faq`** con `FAQPage` schema (ottima per le risposte dirette delle
  AI).
...
2. Categoria principale: _Fornitore di attrezzature per la ristorazione_ (+
   categorie secondarie: arredamento commerciale, riparazione elettrodomestici).
3. NAP **identico** al sito:
   `Matarrese srl — Contrada Popoleto, 70011 Alberobello (BA) — +39 080 4323 431`.
4. Orari, foto dello showroom e dei lavori, descrizione, link al sito.
5. Pubblicare **Post** periodici (eventi, novità) e rispondere alle
   **recensioni**.
...
- Pagine dei **marchi partner** (Lainox, Unox, Rational, Silko, Pedrali, Ciam,
  Qucino…): chiedere di essere elencati come rivenditore/assistenza per la
  Puglia.
...
- **Wikidata**: creare una voce per "Matarrese srl" con dati strutturati aiuta
  le AI e Google a riconoscere l'entità.
...
Puglia", "arredo ristorante Alberobello", ecc. su Google (AI Overviews), ChatGPT
e Perplexity per verificare se e come il sito viene citato.
...
- **Immagini su Vercel**: con l'ottimizzazione riattivata, monitorare le quote
  di _Image Optimization_ su Vercel dopo il deploy (eventualmente CDN/limiti).
